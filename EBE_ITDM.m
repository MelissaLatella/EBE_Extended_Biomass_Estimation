...
%    in the script and comment rows 98-99).
%
% Watning (i): modify row 64 according to the core number of the used pc.
%
%
% Warning (ii): rows 104 to 106 might need to be modified depending 
...
%used to detect double-counted tree tops
...
%%%%folder definition
inputFolder = 'Input_Directory_and_Folder';
inputFormat = '.las';%.las or .csv or .txt are allowed
outputFolder = strcat(inputFolder,'/ITDM_Results');
...
for t = 1 : T
    ...
    pointCloud = readmatrix(strcat(inputFolder,'\',pointCloudFile),'FileType', 'text');
    ...
    writetable(summary, strcat(currentFolder,outputFolder,pointCloudFile,'_StemSpacing_results.txt'))
